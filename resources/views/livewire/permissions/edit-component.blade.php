    ...
    <div class="flex flex-col">
        <div class="recent-activity block">
            <div class="w-full py-2">
                <x-slot name="header">
                    <!-- Section Hero -->
                    @include('tall-acl::header', ['label'=>sprintf("Editar - %s", $model->name),"url"=>route(config('acl.routes.permissions.list'))])
                </x-slot>
            </div>
            <div   class="flex flex-col">
                <div class="mt-5 md:mt-0">
                    <form wire:submit.prevent="saveAndStay">
                        ...
                        @include('tall-acl::livewire.permissions.form')
                        </div>
                        <div class="flex justify-end px-4 py-3 bg-gray-50 text-right sm:px-6 z-10 space-x-2">
                            <x-button wire:loading.attr="disabled" squared negative
                                href="{{ route(config('acl.routes.permissions.list')) }}"
